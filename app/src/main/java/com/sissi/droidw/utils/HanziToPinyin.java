...
//download from https://gist.github.com/D-clock/7a6e33f42c0177439a49d85b73f1e600
...
public class HanziToPinyin {
    ...
    public static class Token {
        ...
        public String toString() {
            return "{type="+type+" src="+source+" target="+target+"}";
        }
    ...
    public static HanziToPinyin getInstance() {
        synchronized (HanziToPinyin.class) {
            ...
            // DEBUG
//            PcTrace.p("####################### BEGIN UNIHANS ##################");
//            for (int i=0; i<UNIHANS.length; ++i){
//                if (0==i%6){
//                    System.out.println("");
//                }
//                System.out.print(" "+UNIHANS[i]);
//            }
//            PcTrace.p("####################### END UNIHANS ##################");

            // Check if zh_CN collation data is available
            final Locale locale[] = Collator.getAvailableLocales();
            for (int i = 0; i < locale.length; i++) {
//                PcTrace.p("locale[%s]=%s", i, locale[i]);
...
//                    PcTrace.p("Locale.getDefault()=%s", Locale.getDefault());
                    sInstance = new HanziToPinyin(true);
                    return sInstance;
                }
            }
...
//                PcTrace.p("letter=%s, unihan=%s, cmp=%s, begin=%s, end=%s offset=%s", letter, unihan, cmp, begin, end, offset);
                if (cmp == 0) {
                    break;
                } else if (cmp > 0) {
                    begin = offset + 1;
...
//        PcTrace.p("unihan=%s, pinyin=%s, offset=%s", UNIHANS[offset], pinyin, offset);
     ...
     * space will be put into a BundleResult, One Hanzi character which has pinyin will be treated as a
     * BundleResult. If these is no China collator, the empty token array is returned.
     */
    public ArrayList<Token> get(final String input) {
        ...
        // a. BundleResult type changed
        // b. Get the Pinyin of current charater.
        // c. current character is space.
//        PcTrace.p("============= BEGIN input name=%s", input);
        for (int i = 0; i < inputLength; i++) {
            final char character = input.charAt(i);
//            PcTrace.p("char=(%s), (last)tokenType=%s, sb=(%s,%s)", character, tokenType, sb, sb.length());
            if (character == ' ') {
                if (sb.length() > 0) {
...
//        PcTrace.p("============= END input name=%s", input);
        return tokens;
    }
